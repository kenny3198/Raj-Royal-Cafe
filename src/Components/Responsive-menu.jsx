...
const ResponsiveMenu = ({open, setOpen}) => {
  return (
   <div className={`${open ? "left-0" : "-left-[100%]"} fixed bottom-0
    top-0 x-30 flex h-screen w-[55%] flex-col justify-between
    bg-amber-950 px-8 pb-6 pt-16 text-black md:hidden
     rounded-r-xl shadow-md transition-all`}>
    <div>
        <div className='flex justify-end text-3xl text-white cursor-pointer' onClick={() => setOpen(false)}>X</div>
                <nav className='mt-12'>
